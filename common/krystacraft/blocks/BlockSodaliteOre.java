...
public class BlockSodaliteOre extends Block {
	...
	public int idDropped(int metadata, Random random, int fortune){
		return Items.sodaliteCrystal.itemID;
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (int i = 0; i < count; i++) {
			...
			if (luckDraw <= 0.25) {
				idDropped = Items.sodaliteCrystalLarge.itemID;
				metaDropped = 1;
			}
			else if (luckDraw <= 1.00) {
				idDropped = Items.sodaliteCrystal.itemID;
				metaDropped = 3;
			}
