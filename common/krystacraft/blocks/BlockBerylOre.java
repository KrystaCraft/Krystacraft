...
public class BlockBerylOre extends Block {
	...
	public BlockBerylOre(int id) {
		super(id, Material.rock);
		
		this.setUnlocalizedName(Strings.BlockBerylOre_unlocalizedName);
	...
	public void registerIcons(IconRegister icon) {
		blockIcon = icon.registerIcon(References.MOD_ID.toLowerCase() + ":block" + Strings.BlockBerylOre_unlocalizedName);
	}
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (int i = 0; i < count; i++) {
			...
			if (luckDraw <= 0.25) {
				idDropped = Items.berylCrystalLarge.itemID;
				metaDropped = 1;
			}
			else if (luckDraw <= 1.00) {
				idDropped = Items.berylCrystal.itemID;
				metaDropped = 3;
			}
