...
public class BlockTourmalineOre extends Block {

	public BlockTourmalineOre(int id) {
		super(id, Material.rock);
		
		this.setUnlocalizedName(Strings.BlockTourmalineOre_unlocalizedName);
	...
	public void registerIcons(IconRegister icon) {
		blockIcon = icon.registerIcon(References.MOD_ID.toLowerCase() + ":block" + Strings.BlockTourmalineOre_unlocalizedName);
	}
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (int i = 0; i < count; i++) {
			...
			if (luckDraw <= 0.25) {
				idDropped = Items.tourmalineCrystalLarge.itemID;
				metaDropped = 1;
			}
			else if (luckDraw <= 1.00) {
				idDropped = Items.tourmalineCrystal.itemID;
				metaDropped = 3;
			}
