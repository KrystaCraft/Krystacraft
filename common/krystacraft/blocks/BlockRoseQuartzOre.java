...
public class BlockRoseQuartzOre extends Block {

	public BlockRoseQuartzOre(int id) {
		super(id, Material.rock);
		
		this.setUnlocalizedName(Strings.BlockRoseQuartzOre_unlocalizedName);
	...
	public void registerIcons(IconRegister icon) {
		blockIcon = icon.registerIcon(References.MOD_ID.toLowerCase() + ":block" + Strings.BlockRoseQuartzOre_unlocalizedName);
	}
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (int i = 0; i < count; i++) {
			...
			if (luckDraw <= 0.25) {
				idDropped = Items.roseQuartzCrystalLarge.itemID;
				metaDropped = 1;
			}
			else if (luckDraw <= 1.00) {
				idDropped = Items.roseQuartzCrystal.itemID;
				metaDropped = 3;
			}
