...
public class BlockMalachiteOre extends Block {
	...
	public BlockMalachiteOre(int id) {
		super(id, Material.rock);
		
		this.setUnlocalizedName(Strings.BlockMalachiteOre_unlocalizedName);
	...
	public void registerIcons(IconRegister icon) {
		blockIcon = icon.registerIcon(References.MOD_ID.toLowerCase() + ":block" + Strings.BlockMalachiteOre_unlocalizedName);
	}
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (int i = 0; i < count; i++) {
			...
			if (luckDraw <= 0.25) {
				idDropped = Items.malachiteCrystalLarge.itemID;
				metaDropped = 1;
			}
			else if (luckDraw <= 1.00) {
				idDropped = Items.malachiteCrystal.itemID;
				metaDropped = 3;
			}
